...
void record_mode(MockEndpoint& endpoint, pg_transfer_mode_t mode) {
    switch (mode) {
    case PG_TRANSFER_AUTO:
        ++endpoint.stats.auto_requests;
        break;
    case PG_TRANSFER_EAGER:
        ++endpoint.stats.eager_requests;
        break;
    case PG_TRANSFER_RENDEZVOUS:
        ++endpoint.stats.rendezvous_requests;
        break;
    default:
        break;
    }
...
void record_post(MockEndpoint& endpoint,
                 RequestKind kind,
                 std::size_t bytes,
                 pg_transfer_mode_t mode) {
    if (kind == RequestKind::Send) {
        ++endpoint.stats.send_requests;
        endpoint.stats.sent_bytes += bytes;
    } else {
        ++endpoint.stats.receive_requests;
        endpoint.stats.received_bytes += bytes;
    }
    record_mode(endpoint, mode);
    ++endpoint.stats.current_outstanding_requests;
    endpoint.stats.maximum_outstanding_requests =
        std::max(endpoint.stats.maximum_outstanding_requests,
                 endpoint.stats.current_outstanding_requests);
}

void record_completion(MockEndpoint& endpoint) {
    if (endpoint.stats.current_outstanding_requests > 0) {
        --endpoint.stats.current_outstanding_requests;
    }
}
...
bool try_complete_receive_locked(RequestState& request) {
    ...
    for (auto iterator = queue.begin(); iterator != queue.end(); ++iterator) {
        ...
        if (request.bytes > 0) {
            std::memcpy(request.receive_buffer,
                        iterator->payload.data(),
                        request.bytes);
        }
        queue.erase(iterator);
        request.status = 0;
...
int mock_post_send_next(pg_transport_t* transport,
    ...
    try {
        if (transport == nullptr || request == nullptr ||
            request->internal != nullptr || (buffer == nullptr && bytes > 0)) {
            return -1;
        }
        ...
        std::unique_ptr<RequestState> state(new (std::nothrow) RequestState());
        if (!state) {
            return -1;
        }
        ...
        const int destination = (endpoint->rank + 1) % endpoint->world_size;
        {
            ...
            endpoint->ring->incoming[static_cast<std::size_t>(destination)]
                .push_back(std::move(message));
        }
        endpoint->ring->condition.notify_all();

        record_post(*endpoint, RequestKind::Send, bytes, mode);
        request->id = endpoint->next_request_id++;
        request->internal = state.release();
        return 0;
    } catch (...) {
        return -1;
    }
...
int mock_post_recv_prev(pg_transport_t* transport,
    ...
    try {
        ...
        record_post(*endpoint, RequestKind::Receive, bytes, mode);
        request->id = endpoint->next_request_id++;
        request->internal = state.release();
        return 0;
    } catch (...) {
...
int mock_test(pg_transport_t* transport,
              pg_request_t* request,
              int* completed) {
    (void)transport;
...
int mock_wait(pg_transport_t* transport, pg_request_t* request) {
    ...
    record_completion(*state->endpoint);
    delete state;
...
extern "C" int mock_transport_group_create(int world_size,
                                             pg_transport_t*** transports) {
    ...
    *transports = nullptr;
    ...
    pg_transport_t** created =
        new (std::nothrow) pg_transport_t*[world_size]{};
    if (created == nullptr) {
        return -1;
    }
...
extern "C" void mock_transport_group_destroy(pg_transport_t** transports,
                                               int world_size) {
...
extern "C" int mock_transport_get_stats(const pg_transport_t* transport,
                                          mock_transport_stats_t* stats) {
    const MockEndpoint* endpoint = endpoint_of(transport);
    if (endpoint == nullptr || stats == nullptr) {
        return -1;
    }
...
extern "C" int mock_transport_reset_stats(pg_transport_t* transport) {
    MockEndpoint* endpoint = endpoint_of(transport);
    if (endpoint == nullptr || endpoint->stats.current_outstanding_requests != 0) {
        return -1;
    }
    endpoint->stats = {};
    return 0;
